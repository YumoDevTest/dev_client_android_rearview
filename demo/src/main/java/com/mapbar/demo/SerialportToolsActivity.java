...
public class SerialportToolsActivity extends Activity {
    ...
    private void openSerialport() {
        ...
        if (connection == null) {
            try {
                connection = SerialPortConnectionCreator.create(SERIALPORT_PATH, 115200);
                connection.start();
                appendLineText("串口打开成功,串口名称=" + SERIALPORT_PATH);
                ...
                e.printStackTrace();
                appendLineText("串口打开失败,串口名称=" + SERIALPORT_PATH + ", 异常=" + e.getMessage());
                return;
            }
        }
    }
